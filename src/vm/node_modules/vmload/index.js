...
function getVmobj(uuid, options, callback)
{
    ...
    if (options.loadManually) {
        ...
        // try Vminfod and fall back to manually loading
        ...
        loadVminfod(function (err, vm) {
            ...
            log.warn({err: err}, 'getVmobj vminfod failure - loading manually');
            loadManually(callback);
        });
    }
            ...
            function (cb) {
                ...
                if (wantField(options, 'last_modified')) {
                    if (cache.last_modified
                        ...
                        vmobj.last_modified =
                            cache.last_modified[vmobj.zonename];
                    } else {
...
function getVmobjs(filter, options, callback)
{
    ...
    if (options.loadManually) {
        ...
        // try Vminfod and fall back to manually loading
        loadVminfod(function (err, vms) {
            if (!err) {
                callback(null, vms);
                return;
            }
            // emit a warning or error message here... vminfod failed
            loadManually(callback);
        });
    }
    ...
    function loadVminfod(done) {
        vminfod.vms(function (err, vms) {
            ...
            async.filterSeries(vms, filter, function (results) {
                done(null, results);
            });
        });
    }
