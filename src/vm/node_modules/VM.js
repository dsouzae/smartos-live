...
var vminfod = require('vminfod/client');
            ...
            vasync.parallel({
                funcs: [
                    function (cb2) {
                        ...
                        var opts = {
                            timeout: 10 * 1000 // TODO don't hardcode
                        };
                        vminfod.waitForEvent(obj, opts, function (e, _) {
                            if (e) {
                                cb2(e);
                                return;
                            }
...
function deleteZone(uuid, log, callback)
{
    ...
    async.series([
        ...
        // TODO: replace these next two with VM.stop(..{force: true} ?
            ...
            if (!vmobj.zonename) {
                // assure deleted
                VM.load(uuid, {fields: ['uuid'], log: log},
                    function (err, obj) {
            ...
            vasync.parallel({
                funcs: [
                    function (cb2) {
                        // XXX for some reason -u <uuid> doesn't work with
                        // delete
...
exports.update = function (uuid, payload, options, callback)
{
    ...
    async.series([
        ...
        // simultaneously blocking on vminfod to give us the necessary "modify"
        // event letting us know it has been successfully updated.
        vasync.parallel({
            funcs: [
                function (cb) {
                    // block on vminfod event
                    ...
                    var opts = {
                        timeout: 30 * 1000 // TODO don't hardcode
                    };
                    vminfod.waitForEvent(obj, opts, function (vm_err, _) {
                        if (vm_err) {
                            cb(vm_err);
                            return;
                        }
            ...
            // we must unlock the lock file regardless of any possible error
            log.debug('releasing lock on ' + lockpath);
            unlock(function (unlock_err) {
                if (unlock_err) {
                    log.error(unlock_err,
